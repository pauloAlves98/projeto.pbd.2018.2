...
public class ControllerCadastroReserva {
	...
	public ControllerCadastroReserva() {
		...
		this.fpv.getBuscarField().setEnabled(false);
		this.fpv.getIrButton().setEnabled(false);
		this.fpv.getAllButton().setEnabled(false);
		this.fpv.getIdField().setEnabled(false);
	...
	private void salvarPJ()
	{
		...
		if(Corrente.usuarioFisico==null && Corrente.usuarioJuridico==null) {
			JOptionPane.showMessageDialog(null,"É preciso que o cliente esteja Logado  para executar esta operação!");
		}else {
			try {
	...
	public void verificaLog() {
		if(Corrente.usuarioFisico!=null) {
			this.fpv.getClienteField().setText(Corrente.usuarioFisico.getLogin());
		}
		else if(Corrente.usuarioJuridico!=null)
			this.fpv.getClienteField().setText(Corrente.usuarioJuridico.getNome());
		else
			this.fpv.getClienteField().setText("");
	}
}
